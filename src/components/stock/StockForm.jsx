...
export default function StockForm({ item, defaultType, onBack, onSave, onDelete, saving }) {
  ...
  // Yeni oluşturulan (veya eski) barkodlarda PWA uyumu için HashRouter formatı (Salt okunur QR Detay sayfası):
  const qrValue     = item?.id ? `${window.location.origin}/#/qr/${item.id}` : 'aerp://new';
