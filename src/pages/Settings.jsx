...
import {
  ...
  Download, Upload, HardDrive, Shield, FolderArchive, RefreshCcw, FileDown, FileUp, AlertTriangle,
} from 'lucide-react';
...
export default function Settings() {
  ...
  const TABS = [
    ...
    ...((profile?.role === ROLES.DEV || profile?.role === ROLES.ADMIN) ? [{ id: 'backup', label: '💾 Yedekleme' }] : []),
  ];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl mx-auto">
      ...
      {/* YEDEKLEME */}
      {activeTab === 'backup' && (
        <BackupManager c={c} currentColor={currentColor} isDark={isDark} />
      )}
    </motion.div>
  );
}
...
function BackupManager({ c, currentColor, isDark }) {
  ...
  const handleBackup = async () => {
    ...
    const modules = backupType === 'full' ? ['full'] : selectedModules;
    if (backupType === 'partial' && modules.length === 0) {
      setError('En az bir modül seçmelisiniz.'); setProcessing(false); return;
    }

    try {
      setProgress('Sunucudan veriler çekiliyor...');
      const res = await fetch('/api/backup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'backup', modules }),
      });
      ...
      const { data } = await res.json();
      setProgress('Yedek dosyası hazırlanıyor...');

      const jsonStr = JSON.stringify(data, null, 2);
      const blob = new Blob([jsonStr], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const dateStr = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
      const typeLabel = backupType === 'full' ? 'TAM' : 'PARCALI';
      const fileName = `AERP_Yedek_${typeLabel}_${dateStr}.json`;

      const a = document.createElement('a');
      a.href = url; a.download = fileName; a.click();
      URL.revokeObjectURL(url);

      const tables = data._manifest?.tables || [];
      const totalRows = tables.reduce((s, t) => s + (data[t]?.length || 0), 0);
      const mediaCount = data._media_files ? Object.keys(data._media_files).length : 0;
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (!data._manifest) throw new Error('Geçersiz yedek dosyası — manifest bulunamadı.');
      ...
      setResult({
        type: 'loaded',
        fileName: file.name,
        created: data._manifest.created_at,
        modules: data._manifest.modules,
        tables: tables.length,
        totalRows,
        mediaFiles: mediaCount,
        availableModules: BACKUP_MODULES.filter(m => m.tables.some(t => data[t]?.length > 0)),
      });
    } catch (err) {
  ...
  const handleRestore = async () => {
    if (!restoreData) { setError('Önce bir yedek dosyası yükleyin.'); return; }
    const modules = restoreType === 'full' ? ['full'] : restoreModules;
    if (restoreType === 'partial' && modules.length === 0) {
      setError('En az bir modül seçmelisiniz.'); return;
    }

    const confirmMsg = restoreType === 'full'
      ? '⚠ TAM GERİ YÜKLEME: Mevcut TÜM veriler silinecek ve yedekteki verilerle değiştirilecek. Bu işlem geri alınamaz!\n\nDevam etmek istiyor musunuz?'
      : `⚠ PARÇALI GERİ YÜKLEME: Seçili ${modules.length} modülün mevcut verileri silinecek ve yedekteki verilerle değiştirilecek.\n\nDevam etmek istiyor musunuz?`;

    if (!window.confirm(confirmMsg)) return;
    if (!window.confirm('Son kez onaylıyor musunuz? Bu işlem GERİ ALINAMAZ!')) return;

    setProcessing(true); setError(null); setResult(null);

    try {
      setProgress('Veriler geri yükleniyor...');
      const res = await fetch('/api/backup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'restore', modules, data: restoreData }),
      });
      ...
      const totalInserted = log.reduce((s, l) => s + (l.inserted || l.uploaded || 0), 0);
      const hasErrors = log.some(l => l.errors?.length > 0);

      setResult({ type: 'restore', log, totalInserted, hasErrors });
      setRestoreData(null); setRestoreFile(null);
    } catch (e) {
      setError('Geri yükleme hatası: ' + e.message);
    } finally {
      setProcessing(false); setProgress('');
    }
  };
  ...
  return (
    <div className="rounded-3xl p-6 sm:p-8 space-y-6" style={{ background: c.card, border: `1px solid ${c.border}` }}>
      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="p-3 rounded-2xl" style={{ background: 'rgba(139,92,246,0.12)' }}>
          <HardDrive size={22} style={{ color: '#8b5cf6' }} />
        </div>
        <div>
          <h2 className="text-lg font-bold" style={{ color: c.text }}>Yedekleme & Geri Yükleme</h2>
          <p className="text-sm mt-0.5" style={{ color: c.muted }}>
            Tüm veritabanı, ayarlar ve medya dosyalarının yedeğini alın veya geri yükleyin.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl p-5 space-y-4"
          style={{ background: isDark ? 'rgba(255,255,255,0.03)' : '#f8fafc', border: `1px solid ${mode === 'backup' ? `${currentColor}50` : c.border}` }}>
          <div className="flex items-center gap-2">
            <FileDown size={18} style={{ color: '#10b981' }} />
            <h3 className="text-sm font-bold" style={{ color: c.text }}>Yedek Al</h3>
          </div>

          <div className="flex gap-2">
            {[{ id: 'full', label: 'Tam Yedek' }, { id: 'partial', label: 'Parçalı Yedek' }].map(t => (
              <button key={t.id} onClick={() => { setBackupType(t.id); setMode('backup'); }}
                className="flex-1 py-2 rounded-xl text-xs font-bold transition-all"
                style={{
                  background: backupType === t.id && mode === 'backup' ? `${currentColor}15` : 'transparent',
                  color: backupType === t.id && mode === 'backup' ? currentColor : c.muted,
                  border: `1px solid ${backupType === t.id && mode === 'backup' ? `${currentColor}40` : c.border}`,
                }}>{t.label}</button>
            ))}
          </div>

          {mode === 'backup' && backupType === 'partial' && (
            <div className="space-y-1.5 max-h-[280px] overflow-y-auto pr-1">
              {BACKUP_MODULES.map(mod => (
                <label key={mod.id}
                  className="flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer transition-all"
                  style={{ background: selectedModules.includes(mod.id) ? `${currentColor}10` : 'transparent',
                           border: `1px solid ${selectedModules.includes(mod.id) ? `${currentColor}30` : 'transparent'}` }}
                  onClick={() => toggleModule(mod.id, setSelectedModules)}>
                  <input type="checkbox" checked={selectedModules.includes(mod.id)} readOnly
                    style={{ accentColor: currentColor }} />
                  <span className="text-sm">{mod.icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold" style={{ color: c.text }}>{mod.label}</p>
                    <p className="text-[10px]" style={{ color: c.muted }}>{mod.tables.join(', ')}</p>
                  </div>
          ...
          <button onClick={handleBackup} disabled={processing}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-all"
            style={{ background: processing ? '#64748b' : '#10b981', opacity: processing ? 0.7 : 1 }}>
            {processing ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
            {processing ? 'Yedekleniyor...' : backupType === 'full' ? 'Tam Yedek Al' : `${selectedModules.length} Modül Yedekle`}
          </button>
        </div>

        {/* Geri Yükle */}
        <div className="rounded-2xl p-5 space-y-4"
          style={{ background: isDark ? 'rgba(255,255,255,0.03)' : '#f8fafc', border: `1px solid ${mode === 'restore' ? `${currentColor}50` : c.border}` }}>
          <div className="flex items-center gap-2">
            <FileUp size={18} style={{ color: '#3b82f6' }} />
            <h3 className="text-sm font-bold" style={{ color: c.text }}>Geri Yükle</h3>
          </div>

          <input ref={fileInputRef} type="file" accept=".json" onChange={handleFileUpload} className="hidden" />
          <button onClick={() => fileInputRef.current?.click()} disabled={processing}
            className="w-full py-3 rounded-xl text-xs font-semibold transition-all border-2 border-dashed"
            style={{ borderColor: restoreFile ? '#10b981' : c.border, color: restoreFile ? '#10b981' : c.muted,
                     background: restoreFile ? 'rgba(16,185,129,0.05)' : 'transparent' }}>
            {restoreFile ? (
              <span className="flex items-center justify-center gap-2">
                <CheckCircle2 size={14} /> {restoreFile}
              </span>
            ) : (
              <span className="flex items-center justify-center gap-2">
                <Upload size={14} /> Yedek dosyası seçin (.json)
              </span>
            )}
          </button>
          ...
          {restoreData && (
            <>
              <div className="flex gap-2">
                {[{ id: 'full', label: 'Tam Yükle' }, { id: 'partial', label: 'Parçalı Yükle' }].map(t => (
                  <button key={t.id} onClick={() => { setRestoreType(t.id); setMode('restore'); }}
                    className="flex-1 py-2 rounded-xl text-xs font-bold transition-all"
                    style={{
                      background: restoreType === t.id ? `${currentColor}15` : 'transparent',
                      color: restoreType === t.id ? currentColor : c.muted,
                      border: `1px solid ${restoreType === t.id ? `${currentColor}40` : c.border}`,
                    }}>{t.label}</button>
                ))}
              </div>

              {restoreType === 'partial' && result?.availableModules && (
                <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">
                  {result.availableModules.map(mod => (
                    <label key={mod.id}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer transition-all"
                      style={{ background: restoreModules.includes(mod.id) ? `${currentColor}10` : 'transparent',
                               border: `1px solid ${restoreModules.includes(mod.id) ? `${currentColor}30` : 'transparent'}` }}
                      onClick={() => toggleModule(mod.id, setRestoreModules)}>
                      <input type="checkbox" checked={restoreModules.includes(mod.id)} readOnly
                        style={{ accentColor: currentColor }} />
                      <span className="text-sm">{mod.icon}</span>
                      <p className="text-xs font-semibold flex-1" style={{ color: c.text }}>{mod.label}</p>
                    </label>
                  ))}
                </div>
              )}
              ...
              <button onClick={handleRestore} disabled={processing}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-all"
                style={{ background: processing ? '#64748b' : '#ef4444', opacity: processing ? 0.7 : 1 }}>
      ...
      {progress && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          style={{ background: `${currentColor}08`, border: `1px solid ${currentColor}20` }}>
          <Loader2 size={16} className="animate-spin" style={{ color: currentColor }} />
          <p className="text-sm font-semibold" style={{ color: currentColor }}>{progress}</p>
        </motion.div>
      )}
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          style={{ background: isDark ? 'rgba(255,255,255,0.03)' : '#f0fdf4', border: '1px solid rgba(16,185,129,0.2)' }}>

          {result.type === 'backup' && (
            <>
              <div className="flex items-center gap-2">
                <CheckCircle2 size={18} style={{ color: '#10b981' }} />
                <p className="text-sm font-bold" style={{ color: '#10b981' }}>Yedekleme Tamamlandı!</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  { l: 'Dosya', v: result.fileName },
                  { l: 'Tablolar', v: result.tables },
                  { l: 'Toplam Kayıt', v: result.totalRows.toLocaleString() },
                  { l: 'Boyut', v: result.size },
                  ...(result.mediaFiles > 0 ? [{ l: 'Medya Dosyası', v: result.mediaFiles }] : []),
                ].map((s, i) => (
                  <div key={i} className="px-3 py-2 rounded-xl" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : '#ffffff', border: `1px solid ${c.border}` }}>
                    <p className="text-[10px] font-bold uppercase" style={{ color: c.muted }}>{s.l}</p>
                    <p className="text-xs font-bold mt-0.5 truncate" style={{ color: c.text }}>{s.v}</p>
                  </div>
                ))}
              </div>
            </>
          ...
          {result.type === 'loaded' && (
            <>
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold" style={{ color: '#3b82f6' }}>Yedek Dosyası Yüklendi</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  { l: 'Oluşturulma', v: result.created ? new Date(result.created).toLocaleDateString('tr-TR') : '-' },
                  { l: 'Tablolar', v: result.tables },
                  { l: 'Toplam Kayıt', v: result.totalRows.toLocaleString() },
                  ...(result.mediaFiles > 0 ? [{ l: 'Medya Dosyası', v: result.mediaFiles }] : []),
                ].map((s, i) => (
                  <div key={i} className="px-3 py-2 rounded-xl" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : '#ffffff', border: `1px solid ${c.border}` }}>
                    <p className="text-[10px] font-bold uppercase" style={{ color: c.muted }}>{s.l}</p>
                    <p className="text-xs font-bold mt-0.5" style={{ color: c.text }}>{s.v}</p>
                  </div>
                ))}
              </div>
              <p className="text-xs" style={{ color: c.muted }}>
                Yukarıdan Tam veya Parçalı yükleme seçin ve geri yükleme butonuna basın.
              </p>
            </>
          )}
          ...
          {result.type === 'restore' && (
            <>
              <div className="flex items-center gap-2">
                <CheckCircle2 size={18} style={{ color: result.hasErrors ? '#f59e0b' : '#10b981' }} />
                <p className="text-sm font-bold" style={{ color: result.hasErrors ? '#f59e0b' : '#10b981' }}>
                  Geri Yükleme {result.hasErrors ? 'Kısmen ' : ''}Tamamlandı!
                </p>
              </div>
              <p className="text-xs font-semibold" style={{ color: c.text }}>
                Toplam {result.totalInserted.toLocaleString()} kayıt geri yüklendi.
              </p>
              <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
                {result.log.map((entry, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg text-xs"
                    ...
                    <span className="font-mono font-bold" style={{ color: c.text }}>{entry.table}</span>
                    <div className="flex items-center gap-2">
                      {entry.status === 'ok' && (
                        <span style={{ color: '#10b981' }}>✓ {entry.inserted || entry.uploaded || 0}{entry.total ? `/${entry.total}` : ''}</span>
                      )}
                      {entry.status === 'skipped' && <span style={{ color: c.muted }}>— atlandı</span>}
                      {entry.errors?.length > 0 && (
                        <span style={{ color: '#f59e0b' }}>⚠ {entry.errors.length} hata</span>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className="flex items-start gap-3 px-4 py-3 rounded-xl"
        style={{ background: isDark ? 'rgba(245,158,11,0.06)' : '#fffbeb', border: '1px solid rgba(245,158,11,0.15)' }}>
        <AlertTriangle size={14} className="mt-0.5 flex-shrink-0" style={{ color: '#f59e0b' }} />
        <div>
          <p className="text-xs font-bold" style={{ color: '#f59e0b' }}>Önemli Uyarılar</p>
          <ul className="text-[11px] mt-1 space-y-0.5 list-disc list-inside" style={{ color: isDark ? '#94a3b8' : '#64748b' }}>
            <li>Tam yedek medya dosyalarını da içerir — dosya boyutu büyük olabilir</li>
            <li>Geri yükleme mevcut verileri <strong>tamamen siler</strong> ve yerine yedekteki verileri yazar</li>
            <li>Geri yüklemeden önce mutlaka güncel bir yedek alın</li>
            <li>Yedek dosyasını güvenli bir yerde saklayın</li>
          </ul>
        </div>
      </div>
    </div>
